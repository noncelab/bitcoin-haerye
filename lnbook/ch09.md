...
마지막으로 밥은 앨리스에게 commitment_signed 메시지를 전송하여 앨리스의 약정에 서명한다. 그런 다음 앨리스는 밥에게 revoke_and_ack을 보내 새 약정을 승인하고 기존 약정을 철회한다. 최종 결과는 앨리스와 밥 모두 채널 상태를 Commitment #4로 옮긴 것이며, Commitent #4는 기존 HTLC를 제거하고 잔액을 업데이트한다. 현재 채널 상태는 그림 14.와 같다. 

...
- [ECDSA](#ecdsa)
  ...
  - [ECDSA의 토대가 되는 타원곡선 암호1(ECC)에 대해 알아봅니다.](#ecdsa의-토대가-되는-타원곡선-암호1ecc에-대해-알아봅니다)
  - [비트코인에서 사용하는 타원곡선: Secp256k1](#비트코인에서-사용하는-타원곡선-secp256k1)
  - [타원곡선 위의 점 덧셈(Point Addition)3](#타원곡선-위의-점-덧셈point-addition3)
  - [secp256k1와 키쌍](#secp256k1와-키쌍)
...
## ECDSA의 토대가 되는 타원곡선 암호<sup>[1](#footnote_1)</sup>(ECC)에 대해 알아봅니다.
타원곡선 암호는 비대칭키 암호화, 다른 말로 공개키 암호 알고리즘 중 하나입니다. 다른 대표적인 비대칭키인 RSA, 디피-헬만 등과 비교했을 때 **타원곡선 암호는 짧은 키 길이와 빠른 연산속도를 가지면서 동일한 수준의 보안 강도를 제공한다는 특징이 있습니다. 따라서 비트코인의 개인키는 64자리 문자열로 짧은 편이며, 트랜잭션 서명도 빠르게 실행할 수 있는것이죠.**
$$
y^2 = x^3 + ax + b
$$
...
<br>
<figure>
<img src="./01_ECDSA/secp256k1-graph-realnum.png" alt="실수체 위에서 정의된 타원곡선 그래프" style="width:100%">
...
첫번째 이미지는 실수체 위에서 정의된 secp256k1, 두번째 이미지는 유한체 $F_{17}$위에서 정의된 secp256k1입니다. 유한체가 무엇이길래 그래프가 선이 아니라 점의 집합으로 표현되는 것일까요?
...
나머지 연산임을 표시하기 위해 $+_f$ 로 표시했습니다. 29를 유한체의 위수인 17로 나눈 나머지인 12는 $F_{17}$에 속하며 ECDSA에서 모든 연산은 이와 같은 나머지 연산으로 이루어집니다. 이 계산법을 따르면 좌표 (5, 8)이 어떻게 유한체 $F_{17}$위에서 정의된 secp256k1 위의 한 점인지 이해가 됩니다. $8^2\%17=13$, $(5^3+7)\%17=13$

<br>

## 타원곡선 위의 점 덧셈(Point Addition)<sup>[3](#footnote_3)</sup>

 이제 타원곡선 위의 점 덧셈에 대해 소개하려고 합니다. 이 과정만 거치면 비트코인에서 개인키와 공개키가 어떻게 생성되는지, 어떻게 공개키로부터 개인키를 유추해낼 수 없는건지, 서명 생성과 서명 검증은 어떻게 수행되는지 알 수 있습니다. 
...
![타원곡선 위에서 같은 점 덧셈 반복](https://github.com/bitcoinbook/bitcoinbook/raw/develop/images/mbc2_0404.png)

위 그래프는 점 G를 계속해서 덧셈하는 과정을 표현한 것입니다. 처음에 G+G=2G가 계산되고 2G+2G=4G, 4G+4G=8G가 계산되는 과정이 그래프에 자세히 표현되어 있습니다. 만약 k=8이라면 $8・G$의 연산 과정이 위 그래프와 같을 것입니다. 여기서 타원곡선 위의 덧셈 연산 특징을 하나 알 수 있습니다. 만약에 8G의 결과 좌표인 (x,y)와 그 시작점인 G, 두 가지를 안다고 했을 때 여러분은 G로부터 몇번 점 덧셈을 해야 (x,y)에 도달할 수 있는지 유추하실 수 있으신가요? 매우 힘들다는 것을 느끼실 수 있을 겁니다. 이렇게 xG = y에서 G와 y값을 안다고 해도 x값을 구할 수 있는 효율적인 알고리즘이 없는 문제를 이산 로그 문제라고 하며 타원곡선의 이러한 특징은 비트코인의 비대칭키 생성 수단으로 선택된 이유 중 하나입니다.
...
## secp256k1와 키쌍
 ...
 - 1(sequence)
을 합친 이름이며, 효과적인 암호를 위한 표준으로써 도메인 파라미터<sup>[4](#footnote_4)</sup>를 가집니다.
...
여기서 비트코인의 개인키란 k값으로 0에서 n사이, 즉 0부터 0xfffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141 사이의 값 중 하나인 정수이고, 비트코인의 공개키는 G에서 점 덧셈을 k번 한 결과로 곡선 위의 한 점 P($x_k$, $y_k$)입니다.  secp256k1 타원 곡선에서 k번 점 덧셈 한 것을 역으로 연산하여 공개키로부터 개인키를 얻어내는 것은 것이 불가능 하고, n값은 우주에 존재하는 원자의 개수만큼 큰 수이기 때문에 원하는 값이 나올때까지 하나씩 연산해 본다고 했을 때 여러분의 1억 세대의 자손이 이어서 하고 있을지도 모릅니다. 비트코인이 이러한 암호학적 특성을 활용하여 설계되었기 때문에 탈취 없이 비트코인 지갑을 이용할 수 있는 것입니다. 
...
![https://www.oreilly.com/api/v2/epubs/9781491902639/files/images/msbt_0501.png](https://www.oreilly.com/api/v2/epubs/9781491902639/files/images/msbt_0501.png)

A지갑에서 B지갑으로 비트코인을 전송한다고 해봅시다. A는 자신이 사용하려고 하는 UTXO의 주인이 맞다는 것을 증명해야 합니다. 위 그림처럼 <서명 생성(sig)> <공개키(PubK)>로 Unlocking Script를 만들어서 UTXO의 Locking Script를 해제해야 증명할 수 있습니다. 바로 이 과정에서 ECDSA의 서명 생성과 서명 검증 이론이 사용됩니다. 
...
다음은 ECDSA의 서명 생성 과정<sup>[5](#footnote_5)</sup>입니다. 
...
이해를 돕기 위해 다음 과정<sup>[6](#footnote_6)</sup>을 천천히 따라와보시길 바랍니다.
...
<br>
<a name="footnote_3">[3]</a> [wikipedia - Elliptic curve point multiplication
](https://en.wikipedia.org/wiki/Elliptic_curve_point_multiplication)
<br>
<a name="footnote_4">[4]</a> [텍사스 대학 Secp256k1-domain-parameters 웹문서](https://www.cs.utexas.edu/users/moore/acl2/manuals/current/manual/index-seo.php/ECURVE____SECP256K1-DOMAIN-PARAMETERS)
<br>
<a name="footnote_5">[5]</a> [Understanding How ECDSA Protects Your Data by snifikino](https://www.instructables.com/Understanding-how-ECDSA-protects-your-data/)
<br>
<a name="footnote_6">[6]</a> [ANDREA CORBELLINI의 블로그 - Elliptic Curve Cryptography ECDH and ECDSA](https://andrea.corbellini.name/2015/05/30/elliptic-curve-cryptography-ecdh-and-ecdsa/#correctness-of-the-algorithm)

...
이 BIP는 검증 시 중복 데이터 해싱을 최소화하고 서명으로 입력값을 다루기 위해 제안되었으며, 버전 0 증인 프로그램에서 서명 검증을 위한 새로운 트랜잭션 다이제스트 알고리즘을 정의한다.

<br>

## 동기

원래 비트코인 스크립트 시스템에는 4개의 ECDSA 서명 검증 코드가 있다: `CHECKSIG`, `CHECKSIGVERIFY`, `CHECKMULTISIG`, `CHECKMULTISIGVERIFY` (”sigops”). Sighash 유형(`ALL`, `NONE`, `SINGLE`, `ANYONECANPAY`)에 따라 트랜잭션의 직렬화된 하위 집합의 이중 SHA256으로 트랜잭션 다이제스트가 생성되고, 서명은 주어진 공개키로 이 다이제스트를 검증한다. 자세한 절차는 비트코인 위키 문서에 설명되어 있다. <sup>[[1]](#footnote_1)</sup>

안타깝게도 원래의 SignatureHash 트랜잭션 다이제스트 알고리즘에는 적어도 두 가지 약점이 있다.

- 각 서명의 검증을 위한 데이터 해싱의 양은 트랜잭션의 크기에 비례한다. 따라서 트랜잭션의 sigops 수가 증가함에 따라 데이터 해싱은 O(n<sup>2</sup>)로 증가한다. 2015년의 평균적인 컴퓨터에서 1MB 블록을 검증하는데는 2초가 걸렸지만, 5569 sigops가 포함된 1MB 트랜잭션은 검증에 25초가 걸릴 수 있다. 이는 재사용이 가능한 “중간 상태(midstate)”를 도입하여 다이제스트 알고리즘을 최적화함으로써 해결할 수 있기 때문에 시간 복잡성은 O(n)이 된다. <sup>[[2]](#footnote_2)</sup><sup>[[3]](#footnote_3)</sup><sup>[[4]](#footnote_4)</sup>
- 이 알고리즘은 입력에 의해 소비되는 비트코인의 양을 포함하지 않는다. 온라인 네트워크 노드는 출력값을 얻기 위해 지정된 트랜잭션을 요청할 수 있으므로 이는 일반적으로 문제가 되지 않는다. 그러나 오프라인 트랜잭션 서명 장치(콜드월렛)의 경우, 입력 금액을 알 수 없기 때문에 정확한 지출 금액과 트랜잭션 수수료를 계산할 수 없다. 이 문제를 해결하기 위해 콜드월렛은 지출되는 전체 트랜잭션을 확보해야 하는데, 이는 경량 에어갭 지갑 구현에 큰 걸림돌이 될 수 있다. 트랜잭션 다이제스트의 일부 입력값을 포함하면 콜드월렛은 신뢰할 수 없는 소스에서 값을 학습하여 트랜잭션에 안전하게 서명할 수 있다. 잘못된 값이 제공되고 서명이 이루어지는 경우 서명은 무효화 되고, 자금이 손실될 수 있다. <sup>[[5]](#footnote_5)</sup>
...
- 그게 아니라면 `hashOutputs`는 `0x0000……0000`의 `uint256`이다. <sup>[[7]](#footnote_7)</sup>

이전 검증에서 계산된 `hashPrevOuts`, `hashSequence`, `hashOutputs`는 동일한 트랜잭션의 다른 입력에 재사용할 수 있으므로 전체 해싱 과정의 시간 복잡도가 O(n<sup>2</sup>)에서 O(n)으로 감소한다.
...
<a name="footnote_7">7. 원래 알고리즘에서는 단일 서명에 대한 입력 인덱스가 출력량보다 크거나 같으면 `0x0000……0001`의 `uint256`이 커밋된다. 이 BIP에서는 의미는 변경하지 않고 `0x0000……0000`이 커밋된다.</a><br/>

...
- [예제](#예제)
  ...
  - [FindAndDelete 없음](#findanddelete-없음)
- [배포](#배포)
- [이전 버전 호환성](#이전-버전-호환성)
- [참조 구현](#참조-구현)
- [참고자료](#참고자료)
...
이 제안은 검증 시 중복 데이터 해싱을 최소화하고 서명으로 입력 값을 다루기 위해 버전 0 증인 프로그램에서 서명 검증을 위한 새로운 트랜잭션 다이제스트 알고리즘을 정의한다.

<br>

## 동기

원래 비트코인 스크립트 시스템에는 4개의 ECDSA 서명 검증 코드가 있다: `CHECKSIG`, `CHECKSIGVERIFY`, `CHECKMULTISIG`, `CHECKMULTISIGVERIFY` (”sigops”). Sighash 유형(`ALL`, `NONE`, `SINGLE`, `ANYONECANPAY`)에 따라 트랜잭션의 직렬화된 하위 집합의 이중 SHA256으로 트랜잭션 다이제스트가 생성되고, 서명은 주어진 공개키로 이 다이제스트를 검증한다. 자세한 절차는 비트코인 위키 문서에 설명되어 있다. [[1](https://github.com/bitcoin/bips/blob/master/bip-0143.mediawiki#cite_note-wiki_1)]

안타깝게도 원래의 SignatureHash 트랜잭션 다이제스트 알고리즘에는 적어도 두 가지 약점이 있다.

- 각 서명의 검증을 위한 데이터 해싱의 양은 트랜잭션의 크기에 비례한다. 따라서 트랜잭션의 sigops 수가 증가함에 따라 데이터 해싱은 O(n<sup>2</sup>)로 증가한다. 2015년의 평균적인 컴퓨터에서 1MB 블록을 검증하는데는 2초가 걸렸지만, 5569 sigops가 포함된 1MB 트랜잭션은 검증에 25초가 걸릴 수 있다. 이는 재사용이 가능한 “중간 상태(midstate)”를 도입하여 다이제스트 알고리즘을 최적화함으로써 해결할 수 있기 때문에 시간 복잡성은 O(n)이 된다. [[2](https://github.com/bitcoin/bips/blob/master/bip-0143.mediawiki#cite_note-2)][[3](https://github.com/bitcoin/bips/blob/master/bip-0143.mediawiki#cite_note-3)][[4](https://github.com/bitcoin/bips/blob/master/bip-0143.mediawiki#cite_note-4)]
- 이 알고리즘은 입력에 의해 소비되는 비트코인의 양을 포함하지 않는다. 온라인 네트워크 노드는 출력값을 얻기 위해 지정된 트랜잭션을 요청할 수 있으므로 이는 일반적으로 문제가 되지 않는다. 그러나 오프라인 트랜잭션 서명 장치(콜드월렛)의 경우, 입력 금액을 알 수 없기 때문에 정확한 지출 금액과 트랜잭션 수수료를 계산할 수 없다. 이 문제를 해결하기 위해 콜드월렛은 지출되는 전체 트랜잭션을 확보해야 하는데, 이는 경량 에어갭 지갑 구현에 큰 걸림돌이 될 수 있다. 트랜잭션 다이제스트의 일부 입력값을 포함하면 콜드월렛은 신뢰할 수 없는 소스에서 값을 학습하여 트랜잭션에 안전하게 서명할 수 있다. 잘못된 값이 제공되고 서명이 이루어지는 경우 서명은 무효화 되고, 자금이 손실될 수 있다. [[5](https://github.com/bitcoin/bips/blob/master/bip-0143.mediawiki#cite_note-5)]

앞서 언급한 수정사항을 원본 스크립트 시스템에 배포하는 것은 간단한 작업이 아니다. 하드포크를 하거나, 스택 항목을 제거하거나 삽입할 수 없는 새로운 sigops를 위한 소프트포크를 해야 한다. 하지만 세그윗 소프트포크의 도입은 업그레이드 되지 않은 노드가 항상 해당 트랜잭션 출력을 임의의 서명 또는 서명이 전혀 없는 것으로 간주하기 때문에 기존의 시스템을 방해하지 않고도 다른 스크립트 의미 집합을 정의할 수 있는 기회를 제공한다.

<br>

## 명세

새로운 트랜잭션 다이제스트 알고리즘이 정의되었지만 버전 0 증인 프로그램의 sigops에만 적용된다.
...
다음 직렬화의 이중 SHA256:
  ...
  5. 입력의 scriptCode (CTxOuts 내부에서 스크립트로 직렬화됨)
  6. 입력이 소비한 output (8바이트 리틀 엔디안)
  7. 입력의 nSequence (4바이트 리틀 엔디안)
  8. hashOutputs (32바이트 해시)
  9. 트랜잭션의 nLocktime (4바이트 리틀 엔디안)
  10. sighash 유형 (4바이트 리틀 엔디안)
```

<br>

원래의 sighash 유형의 의미는 다음을 제외하고는 변경되지 않는다.

1. 직렬화 방식이 변경됨
2. 모든 sighash 유형이 서명된 입력에 의해 소비되는 양을 커밋함
3. 서명의 `FindAndDelete`가 `scriptCode`에 적용되지 않음
4. `OP_CODESEPARATOR` 이후의 `OP_CODESEPARATOR`(s)는 `scriptCode`에서 제거되지 않음. (마지막으로 실행된 `OP_CODESEPARATOR`와 그 이전의 모든 스크립트는 항상 제거됨)
5. `SINGLE`은 입력 인덱스에 커밋하지 않음. `ANYONECANPAY`가 설정되지 않은 경우, `hashPrevouts`와 `outpoint`가 함께 입력 인덱스에 암시적으로 커밋되므로 의미는 변경되지 않음. `SINGLE`을 `ANYONECANPAY`와 함께 사용하면 인덱스 커밋을 생략해도 각 쌍이 동등한 인덱스에 위치하는 한 입력-출력 쌍의 순열이 허용됨.

항목 1, 4, 7, 9, 10은 원래의 알고리즘과 동일한 의미를 갖는다. [[1](https://github.com/bitcoin/bips/blob/master/bip-0143.mediawiki#cite_note-wiki_1)]

항목 5:

- P2WPKH 증인 프로그램의 경우, `scriptCode`는 `0x1976a914{20-byte-pubkey-hash}88ac`이다.
- P2WSH 증인 프로그램의 경우,
    - `witnessScript`에 `OP_CODESEPARATOR`가 포함되지 않은 경우, `scriptCode`는 `CTxOut` 내부에서 스크립트로 직렬화된 `witnessScript`이다.
    - `witnessScript`에 `OP_CODESEPARATOR`가 포함되어 있는 경우, `scriptCode`는 서명 확인 opcode가 실행되기 전에 마지막으로 실행된 `OP_CODESEPARATOR`를 포함한 모든 것을 제거한, 그리고 `CTxOut` 내부에 스크립트로 직렬화된 `witnessScript`이다. (정확한 의미는 아래 예시에서 확인할 수 있다.)

항목 6은 이 입력에 사용된 비트코인의 양을 나타내는 8바이트 값이다.

`hashPrevouts`

- 만약 `ANYONECANPAY` 플래그가 설정되지 않은 경우, `hashPrevouts`는 모든 입력 아웃포인트 직렬화의 이중 SHA256 값이다.
- 그게 아니라면 `hashPrevouts`는 `0x0000……0000`의 `uint256`이다.

`hashSequence`

- `ANYONECANPAY`, `SINGLE`, `NONE` sighash 유형 중 아무것도 설정되지 않은 경우, `hashSequence`는 모든 입력의 `nSequence` 직렬화의 이중 SHA256 값이다.
- 그게 아니라면 `hashSequence`는 `0x0000……0000`의 `uint256`이다.

`hashOutputs`

- sighash 유형이 `SINGLE`도 `NONE`도 아닌 경우, `hashOutputs`는 모든 출력량(8바이트 리틀 엔디안) 직렬화(CTxOuts 내부에서 스크립트로 직렬화됨)를 `scriptPubKey`와 함께 이중 SHA256 값으로 반환한다.
- sighash 유형이 `SINGLE`이고, 입력 인덱스가 출력 개수보다 작은 경우 `hashOutputs`는 입력과 동일한 `scriptPubKey`를 가진 출력량의 이중 SHA256 값이다.
- 그게 아니라면 `hashOutputs`는 `0x0000……0000`의 `uint256`이다. [[7](https://github.com/bitcoin/bips/blob/master/bip-0143.mediawiki#cite_note-7)]

이전 검증에서 계산된 `hashPrevOuts`, `hashSequence`, `hashOutputs`는 동일한 트랜잭션의 다른 입력에 재사용할 수 있으므로 전체 해싱 과정의 시간 복잡도가 O(n^2)에서 O(n)으로 감소한다.
...
기본 정책으로 `P2WPKH` 및 `P2WSH`에서는 압축된 공개키만 허용된다. 버전 0 증인 프로그램 내에서 sigop에 전달되는 각 공개키는 압축된 키여야 하며, 첫 바이트는 `0x02` 또는 `0x03`이어야 하고, 크기는 33바이트여야 한다. 이 규칙을 위반하는 트랜잭션은 기본적으로 **릴레이** 되거나 채굴되지 않는다.
...
합의에 중요한 동작의 일관성을 보장하기 위해 개발자는 아래의 모든 테스트에 대해 구현 테스트를 해야 한다. 이 제안과 관련된 더 많은 테스트는 https://github.com/bitcoin/bitcoin/tree/master/src/test/data에서 확인할 수 있다.
...
다음은 서명되지 않은 트랜잭션이다:
  ...
  직렬화된 서명된 트랜잭션은 다음과 같다: 
...
다음은 서명되지 않은 트랜잭션이다: 0100000001db6b1b20aa0fd7b23880be2ecbd4a98130974cf4748fb66092ac4d3ceb1a54770100000000feffffff02b8b4eb0b000000001976a914a457b684d7f0d539a46a45bbc043f35b59d0d96388ac0008af2f000000001976a914fd270b1ee6abcaea97fea7ad0402e8bd8ad6d77c88ac92040000
  ...
  직렬화된 서명된 트랜잭션은 다음과 같다: 
    nVersion:  01000000
    marker:    00
    flag:      01
...
이 예시는 `OP_CODESEPARATOR`와 범위를 벗어난 `SIGHASH_SINGLE`이 어떻게 처리되는지 보여준다.
...
다음은 서명되지 않은 트랜잭션이다:
  ...
  직렬화된 서명된 트랜잭션은 다음과 같다: 
```

이 예시는 실행되지 않은 OP_CODESEPARATOR가 어떻게 처리되는지 보여주며, SINGLE|ANYONECANPAY는 입력 인덱스에 커밋하지 않는다.
...
다음은 서명되지 않은 트랜잭션이다:
  ...
  scriptCode:  270063ab68210392972e2eb617b2388771abe27235fd5ac44af8e61693261550447a4c3e39da98ac
               (OP_CODESEPARATOR가 실행되지 않으므로 scriptCode에서 아무것도 제거되지 않는다.)
  ...
  SINGLE|ANYONECANPAY는 입력 인덱스에 커밋하지 않으므로 입력-출력 쌍이 바뀌어도 서명은 여전히 유효하다:
...
이 예시는 6개의 서로 다른 SIGHASH 유형으로 서명된 P2SH-P2WSH 6-of-6 다중 서명 증인 프로그램이다.
...
다음은 서명되지 않은 트랜잭션이다: 010000000136641869ca081e70f394c6948e8af409e18b619df2ed74aa106c1ca29787b96e0100000000ffffffff0200e9a435000000001976a914389ffce9cd9ae88dcc0631e88a821ffdbe9bfe2688acc0832f05000000001976a9147480a33f950689af511e6e84c138dbbd3c3ee41588ac00000000
  ...
  입력은 P2SH-P2WSH 6대 6 다중 서명 증인 프로그램에서 가져온다:
  ...
  직렬화된 서명된 트랜잭션은 다음과 같다: 
```
...
이 예시들은 서명에 대한 `FindAndDelete`가 적용되지 않았음을 보여준다. 트랜잭션은 일반적인 방식이 아닌 다른 방식으로 생성된다. 개인키를 사용하여 서명하는 대신 서명은 `witnessScript`의 일부로 미리 결정된다. 공개키는 이 제안에 정의된 고정 서명과 `sighash`를 사용하여 키 복구를 통해 생성된다. 따라서 개인키는 알 수 없다.
...
새로운 직렬화 형식은 BIP-144에 정의되어 있다. [8](https://github.com/bitcoin/bips/blob/master/bip-0143.mediawiki#cite_note-8)
...
소프트포크로서 이전 소프트웨어는 수정 없이 계속 작동한다. 그러나 업그레이드 되지 않은 노드는 증인 데이터를 보거나 검증하지 않으며, 재정의된 sigops를 포함한 모든 증인 프로그램을 누구나 사용할 수 있는 스크립트로 간주한다.
...
1. [https://en.bitcoin.it/wiki/OP_CHECKSIG](https://en.bitcoin.it/wiki/OP_CHECKSIG)
2. [CVE-2013-2292](https://web.nvd.nist.gov/view/vuln/detail?vulnId=CVE-2013-2292)
3. [New Bitcoin vulnerability: A transaction that takes at least 3 minutes to verify](https://bitcointalk.org/?topic=140078)
4. [The Megatransaction: Why Does It Take 25 Seconds?](http://rusty.ozlabs.org/?p=522)
5. [SIGHASH_WITHINPUTVALUE: Super-lightweight HW wallets and offline data](https://bitcointalk.org/index.php?topic=181734.0)
6. [BIP141: Segregated Witness (Consensus layer)](https://github.com/bitcoin/bips/blob/master/bip-0141.mediawiki)
7. 원래 알고리즘에서는 단일 서명에 대한 입력 인덱스가 출력량보다 크거나 같으면 `0x0000……0001`의 `uint256`이 커밋된다. 이 BIP에서는 의미는 변경하지 않고 `0x0000……0000`이 커밋된다.
8. [BIP144: Segregated Witness (Peer Services)](https://github.com/bitcoin/bips/blob/master/bip-0144.mediawiki)

...
- 각 서명의 검증을 위한 데이터 해싱의 양은 트랜잭션의 크기에 비례한다. 따라서 트랜잭션의 sigops 수가 증가함에 따라 데이터 해싱은 O(n<sup>2</sup>)로 늘어난다. 2015년의 평균적인 컴퓨터를 기준으로 1MB 블록을 검증하는데는 2초가 걸리지만, 5569 sigops가 포함된 1MB 트랜잭션은 검증에 25초가 걸릴 수 있다. 이는 재사용이 가능한 “중간 상태(midstate)”를 도입하여 다이제스트 알고리즘을 최적화함으로써 해결할 수 있기 때문에 복잡도는 O(n)이 된다. <sup>[[2]](#footnote_2)</sup><sup>[[3]](#footnote_3)</sup><sup>[[4]](#footnote_4)</sup>
...
// 버전(Version)
ss << txTo.nVersion;
// prevouts/nSequence 입력 (flags에 따라 none/all)
ss << hashPrevouts;
ss << hashSequence;
// 서명중인 입력 (scriptSig를 scriptCode + amount로 대체)
// prevout은 hashPrevout에, nSequence는 hashSequence에 각각 포함되어 있을 수 있다.
ss << txTo.vin[nIn].prevout;
ss << static_cast<const CScriptBase&>(scriptCode);
ss << amount;
ss << txTo.vin[nIn].nSequence;
// Outputs (flags에 따라 none/one/all)
ss << hashOutputs;
// Locktime
ss << txTo.nLockTime;
// Sighash 유형
...
```
다음 트랜잭션은 OP_CHECKMULTISIGVERIFY 버전의 FindAndDelete 예제이다.
